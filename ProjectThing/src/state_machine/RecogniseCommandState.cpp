...
void RecogniseCommandState::enterState()
{
    ...
    // turn on listening (green) LED
    digitalWrite(GPIO_NUM_4, false);
...
bool RecogniseCommandState::run()
{
    ...
    if (sample_count > 0)
    {
        ...
        {
            // turn on evaluating (red) LED
            digitalWrite(GPIO_NUM_4, true);
            digitalWrite(GPIO_NUM_26, false);
            digitalWrite(GPIO_NUM_25, false);
            ...
            Serial.println("Intent result: ");
            Serial.println(intentResult);
